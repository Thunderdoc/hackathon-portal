...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
        ...
        fetchData('http://localhost:5001/api/admin/messages', setMessages);
        fetchData('http://localhost:5001/api/mentorship/queue', (data) => setMentorshipTickets(data || []));
        fetchData('http://localhost:5001/api/admin/logs', setLogs);
        ...
        try {
            const rRes = await axios.get('http://localhost:5001/api/config/results');
            setResultsReleased(rRes.data.released);
        } catch (e) { }

        fetchData('http://localhost:5001/api/admin/judges', setJudges);
        fetchData('http://localhost:5001/api/admin/judge-stats', setJudgeStats);
        fetchData('http://localhost:5001/api/admin/leaderboard', setLeaderboard);
    };
    ...
    const addLog = async (action) => {
        try { await axios.post('http://localhost:5001/api/admin/log', { action }); } catch (e) { }
    };
    ...
    const handleDeleteTeam = async (id) => {
        if (confirm('CRITICAL: Delete this team PERMANENTLY?')) {
            await axios.delete(`http://localhost:5001/api/admin/team/${id}`);
    ...
    const toggleLockdown = async () => {
        ...
        await axios.post('http://localhost:5001/api/config/lockdown', { value: newState.toString() });
    ...
    const handleBroadcast = async () => {
        ...
        try {
            await axios.post('http://localhost:5001/api/admin/broadcast', { message: broadcastMsg });
            setBroadcastMsg('');
    ...
    const handleEmailDispatch = async () => {
        ...
        try {
            await axios.post('http://localhost:5001/api/admin/email', { subject: 'Hackathon Update', body: broadcastMsg });
    ...
    const fetchJudges = async () => {
        const res = await axios.get('http://localhost:5001/api/admin/judges');
        setJudges(res.data);
    };

    const markAsRead = async (id) => {
        await axios.put(`http://localhost:5001/api/admin/message/${id}/read`);
        fetchDashboardData();
    };
    ...
    const handleReplySend = async () => {
        ...
        try {
            if (replyModal.type === 'JUDGE') {
                await axios.post('http://localhost:5001/api/admin/reply-judge', { judgeId: replyModal.id, message: replyMsg });
            } else {
                await axios.post('http://localhost:5001/api/admin/reply', { teamId: replyModal.id, message: replyMsg });
            }
            setReplyModal(null);
            setReplyMsg('');
            alert('Reply Sent');
    ...
    const saveChanges = async () => {
        ...
        try {
            ...
            await axios.put(`http://localhost:5001/api/admin/team/${editingTeam.id}`, finalData);
            alert('Changes Saved!');
            setEditingTeam(null);
            fetchDashboardData();
    ...
    const handleImpersonate = async (teamId) => {
        try {
            const res = await axios.post('http://localhost:5001/api/admin/impersonate', { teamId });
        ...
        <div style={{ minHeight: '100vh', background: '#f8f9fa' }}>
            ...
            <div style={{ background: '#1d3557', padding: '1rem 2rem', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.2)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        <button onClick={() => window.open('http://localhost:5001/api/export', '_blank')} style={{ background: '#212529', border: 'none', borderRadius: '5px', padding: '0.4rem 0.8rem', color: 'white', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '0.4rem', cursor: 'pointer' }}>
                            <Download size={16} /> EXPORT DATA
                        </button>
                        <button onClick={() => window.open('http://localhost:5001/api/admin/backup', '_blank')} title="Download Backup" style={{ background: '#4cc9f0', border: 'none', borderRadius: '5px', padding: '0.4rem 0.8rem', color: '#1d3557', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '0.4rem', cursor: 'pointer' }}><Download size={16} /> BACKUP</button>
            ...
            < div style={{ background: 'white', padding: '1rem 2rem', borderBottom: '1px solid #eee', display: 'flex', gap: '1rem', alignItems: 'center', justifyContent: 'center' }}>
                <button onClick={async () => {
                    ...
                    if (msg) await axios.post('http://localhost:5001/api/config/ticker', { message: msg });
                ...
                <button onClick={async () => {
                    if (confirm(resultsReleased ? "HIDE RESULTS?" : "PUBLISH RESULTS? This will be visible to all teams.")) {
                        await axios.post('http://localhost:5001/api/config/results', { released: !resultsReleased });
                        ...
                        <div className="glass-panel" style={{ padding: '1.5rem', background: 'white', borderRadius: '10px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>
                            ...
                            <div style={{ display: 'grid', gap: '0.5rem', marginBottom: '1rem' }}>
                                ...
                                <button onClick={async () => {
                                    ...
                                    try {
                                        await axios.post('http://localhost:5001/api/admin/judge', newJudge);
                            ...
                            <div style={{ maxHeight: '150px', overflowY: 'auto', borderTop: '1px solid #eee', paddingTop: '0.5rem' }}>
                                ...
                                {judges.map(j => (
                                    <div key={j.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.8rem', padding: '0.3rem', background: '#f8f9fa', marginBottom: '0.3rem', borderRadius: '4px' }}>
                                        ...
                                        <button onClick={async () => { if (confirm('Revoke?')) { await axios.delete(`http://localhost:5001/api/admin/judge/${j.id}`); fetchDashboardData(); } }} style={{ border: 'none', background: 'none', color: '#e63946', cursor: 'pointer' }}><Trash2 size={14} /></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="glass-panel" style={{ padding: '1.5rem', background: 'white', borderRadius: '10px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>
                            ...
                            <div style={{ display: 'grid', gap: '0.5rem' }}>
                                ...
                                <button onClick={async () => {
                                    ...
                                    fd.append('file', f);
                                    try {
                                        await axios.post('http://localhost:5001/api/admin/resource', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    ...
                    <div className="glass-panel" style={{ padding: '1rem', height: '300px', overflowY: 'auto', fontFamily: 'monospace', fontSize: '0.9rem', background: '#212529', color: '#00ff9d', borderRadius: '10px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem', color: '#fff', borderBottom: '1px solid #444', paddingBottom: '0.5rem' }}>
                            ...
                            <button onClick={() => window.open('http://localhost:5001/api/admin/logs/download', '_blank')} style={{ background: 'none', border: '1px solid #00ff9d', color: '#00ff9d', fontSize: '0.7rem', padding: '0.2rem 0.5rem', borderRadius: '3px', cursor: 'pointer' }}>DOWNLOAD CSV</button>
                        </div>
                        {logs.map((log, i) => (
                                    ...
                                    <div key={t.id} style={{ padding: '0.5rem', background: '#fff0f3', border: '1px solid #ffccd5', borderRadius: '5px', marginBottom: '0.5rem' }}>
                                        ...
                                        <button
                                            onClick={async () => {
                                                ...
                                                try {
                                                    await axios.post(`http://localhost:5001/api/mentorship/resolve/${t.id}`);
                        ...
                        <div style={{ background: '#fff', padding: '2rem', borderRadius: '10px', width: '600px', maxHeight: '90vh', overflowY: 'auto' }}>
                            ...
                            <div style={{ marginBottom: '1rem' }}>
                                ...
                                <div style={{ display: 'flex', gap: '1rem' }}>
                                    ...
                                    <button onClick={async () => { if (!confirm("Wipe all judge scores?")) return; try { await axios.delete(`http://localhost:5001/api/admin/score/${editingTeam.id}`); alert("Scores wiped."); } catch (e) { alert("Failed"); } }} style={{ background: '#e63946', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', padding: '0 1rem', whiteSpace: 'nowrap' }}>WIPE JUDGES</button>
                                </div>
                            </div>
