...
const JudgeDashboard = () => {
    ...
    const fetchChat = async (judgeId) => {
        try {
            const res = await axios.get(`http://localhost:5001/api/judge/messages/${judgeId}`);
    ...
    const sendChatMessage = async () => {
        ...
        try {
            await axios.post('http://localhost:5001/api/judge/message', { judgeId: judge.id, message: chatInput });
            setChatInput('');
            fetchChat(judge.id);
        } catch (e) {
    ...
    const fetchCriteria = async () => {
        try {
            const res = await axios.get('http://localhost:5001/api/criteria');
    ...
    const submitScore = async () => {
        try {
            ...
            await axios.post('http://localhost:5001/api/judge/score', {
                judgeId: judge.id,
                                    ...
                                    <div style={{ display: 'flex', gap: '1rem', marginTop: '2rem', paddingTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                                        <button
                                            onClick={async () => {
                                                ...
                                                if (reason) {
                                                    try {
                                                        await axios.post('http://localhost:5001/api/judge/flag', { teamId: selectedTeam.id, reason, judgeId: judge.id });
                                        ...
                                        <button
                                            onClick={async () => {
                                                if (confirm("NOMINATE this team for the Golden Buzzer? (Exceptional Performance)")) {
                                                    try {
                                                        await axios.post('http://localhost:5001/api/judge/nominate', { teamId: selectedTeam.id, judgeId: judge.id });
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '1.5rem' }}>
                            {resources.map(r => (
                                <div key={r.id} style={{ background: 'rgba(255,255,255,0.05)', padding: '1.5rem', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                    ...
                                    <a href={`http://localhost:5001/uploads/${r.filename}`} target="_blank" rel="noreferrer" style={{
